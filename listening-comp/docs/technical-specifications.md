...
*   **3.1. Backend Modules/Components:**
    ...
    *   **3.1.5. `llm_json_generator.py` - Structured JSON Generator (LLM-Powered):**
        ...
        *   **Example JSON Output Schema:**
            ...
            ```json
            {
              ...
              "speakers_info": ["Speaker 1", "Speaker 2"],
              "topics": ["daily_life", "greetings", "social_interactions"],
              "difficulty_level": "A2"
            }
            ```
    ...
    *   **3.1.6. `data_manager.py` - Data Management and Storage (Vector DB Integration):**
        ...
        *   **Data Structure (Vector DB Entries):** Each entry (document or item) in the Vector DB should store the following information for each exercise sequence:
            *   `json_data`: The complete structured JSON object for the exercise sequence (as generated by `llm_json_generator.py`), containing the `dialogue`, `question`, `answers`, `correct_answer_index`, `speakers_info`, `topics`, and `difficulty_level`.
...
**4. Vector DB Integration and Topic-Based Learning**

*   **4.1. Vector DB Structure:**
    *   **Sequence Data Storage:**
        - Store each processed sequence as a document in Vector DB
        - Include full dialogue text, question, answers, and metadata
        - Generate embeddings for topic matching
        - Link to corresponding audio files
    *   **Document Structure:**
        ```json
        {
            "id": "video_id_sequence_1",
            "dialogue": [...],
            "question": "...",
            "answers": [...],
            "correct_answer_index": 0,
...
*   **4.2. Topic-Based Sequence Retrieval:**
    *   **Topic Selection:**
        - Predefined list of common topics (e.g., "Daily Life", "Work", "Travel")
        - Topic mapping to semantic embeddings
        - Optional topic tags for enhanced matching
    *   **Search and Retrieval:**
        - Convert selected topic to embedding vector
        - Perform semantic similarity search in Vector DB
        - Return most relevant sequences
        - Include metadata and audio file paths
    *   **Sequence Ordering:**
        - Sort by relevance score
        - Consider difficulty progression
        - Track previously shown sequences
        - Implement spaced repetition (optional)

*   **4.3. Learning Session Flow:**
    *   **Session Initialization:**
        - User selects learning topic
        - System queries Vector DB for relevant sequences
        - Preload first N sequences for smooth experience
    *   **Sequence Presentation:**
        - Display dialogue text
        - Load and prepare audio playback
...
**5. Interactive UI Implementation (Current Focus)**

*   **5.1. Session State Management:**
    *   **State Variables:**
        - `current_segment_index`: Track current exercise segment
        - `user_answers`: Dictionary mapping segment indices to user's selected answers
        - `score_tracking`: Dictionary storing correct/incorrect answers per segment
        - `exercise_history`: List of completed segments with timestamps
        - `audio_state`: Track audio playback state (playing/paused, current time)
    *   **State Persistence:**
        - Use Streamlit's session_state to maintain state between reruns
        - Implement state initialization on first run
        - Handle state updates during exercise progression

*   **5.2. Interactive Exercise Components:**
    *   **Answer Submission System:**
        - Radio button group for answer options
        - Submit button with validation
        - Immediate feedback display
        - Score tracking update
        - Next segment activation
    *   **Progress Indicators:**
        - Progress bar showing segment completion
        - Score display (correct/total)
        - Current segment indicator
        - Time spent on current segment
    *   **Navigation Controls:**
        - Next/Previous segment buttons
        - Segment selection dropdown
        - Restart exercise button
        - Review mode toggle

*   **5.3. Exercise Flow Control:**
    *   **Progression Rules:**
        - Require answer submission before proceeding
        - Allow review of completed segments
        - Optional retry for incorrect answers
    ...
    *   **Error Prevention:**
        - Confirm before restart/exit
        - Prevent accidental segment skipping
        - Validate answer selection

*   **5.4. Performance Optimization:**
    *   **State Updates:**
        - Minimize full page rerenders
        - Cache segment data
...
*   **6.1. Interactive Features Testing:**
